...
var forms = function forms() {
  ...
  var phoneInputs = document.querySelectorAll('input[name="user_phone"]'); //Удаляем все значения, которые будут не числами
  ...
  form.forEach(function (item) {
    item.addEventListener('submit', function (event) {
      event.preventDefault(); //Создаём блок, в котором показываем, что что-то пошло не так, либо запрос отправился

      var statusMessage = document.createElement('div');
      statusMessage.classList.add('status'); //Помещаем блок в конец формы

      item.appendChild(statusMessage); //Собираем все введенные данные из нашей формы

      var formData = new FormData(item); //Отправляем запрос на сервер по адресу с данными, которые получил formData
...
var modals = function modals() {
  function bindModal(triggerSelector, modalSelector, closeSelector) {
    ...
    var trigger = document.querySelectorAll(triggerSelector); //Модальное окно, которое будет показываться

    var modal = document.querySelector(modalSelector); //Крестик внутри модального окна, нажимая на который, модальное окно будет закрываться 

    var close = document.querySelector(closeSelector); //item - каждый триггер, на который будем вешать обработчик событий
  ...
  bindModal('.phone_link', '.popup', '.popup .popup_close'); //Рассчитать стоимость

  bindModal('.popup_calc_btn', '.popup_calc', '.popup_calc_close'); //Модальное окно,появляющееся каждые 60 сек
...
var tabs = function tabs(headerSelector, tabSelector, contentSelector, activeClass) {
  ...
  hideTabContent();
  showTabContent(); //Вешаем обработчик события на общую область, которая соединяет в себе все табы

  header.addEventListener('click', function (event) {
    var target = event.target;

    if (target && ( //Проверяем, что кликнули на один из табов
